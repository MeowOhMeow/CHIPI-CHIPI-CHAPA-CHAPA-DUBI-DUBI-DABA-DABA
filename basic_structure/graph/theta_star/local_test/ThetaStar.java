...
import Debug_new.Path.*;
import Debug_new.Path.Pair;
        ...
        for (Obstacle obstacle : obstacles) {
            for (double step = 0; step <= 1; step += 0.001) { // Adjust step size if necessary
                ...
                if (x > obstacle.minX - koz && x < obstacle.maxX + koz && y > obstacle.minY - koz
                    ...
                    // Log.i(TAG, "--------------------collision location: " + x + ", " + y + ", " +
                    // z + "---------------------");
                    return false;
                }
            }
        ...
        while (!open.isEmpty()) {
            ...
            for (int neighbor : graph.getNeighbors(currentVertex)) {
                ...
                if (pp != -1 && lineOfSight(new Vertex(pp), new Vertex(neighbor), graph, obstacles, koz)) {
                    double dx = graph.getVertexProperty(neighbor).getValue().getX()
                            - graph.getVertexProperty(new Vertex(pp)).getValue().getX();
                    double dy = graph.getVertexProperty(neighbor).getValue().getY()
                            - graph.getVertexProperty(new Vertex(pp)).getValue().getY();
                    double dz = graph.getVertexProperty(neighbor).getValue().getZ()
                            - graph.getVertexProperty(new Vertex(pp)).getValue().getZ();
