...
import  jp.jaxa.iss.kibo.rpc.sampleapk.Debugging_new;
...
public class YourService extends KiboRpcService {
    ...
    private void goToTakeAPic(int area) {
        ...
        // Get a camera image.
        Log.i("CHIPI-CHIPI", "begin " );
        Mat image = api.getMatNavCam();
        Debugging_new debug=new Debugging_new();
        int[] count = new int[10];
        count=debug.image(image);
        for(int i = 0; i < count.length; ++i)
        {
            if(count[i] != 0)
                api.setAreaInfo(area, MapOfItem(i), count[i]);
        }
        Log.i("CHIPI-CHIPI", "end " );
